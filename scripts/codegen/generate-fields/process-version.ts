...
function renderVersionIndex(
  ...
  const exports = exportModulePaths.map((modulePath) => `export * from "./${modulePath}";`).join("\n");
...
export async function processVersion(inputVersion: string, runtime: RuntimeContext) {
  ...
  const tempVersionDir = join(runtime.outputDir, `.v${short}.tmp`);
  ...
  const exportModulePaths = moduleByIndex.filter((modulePath): modulePath is string => modulePath != null);
  ...
  console.log(`[v${short}] wrote ${join(versionDir, "index.ts")} (${exportModulePaths.length} modules)`);
