...
interface HeadingInferenceConfig {
  /**
   * Whether to allow empty lines between comment lines in the heading.
   * When false, any empty line between comments will mark the end of the heading.
   * @default true
   */
  allowEmptyLinesBetweenComments?: boolean;
...
/**
 * Improved version of inferHeadingFromAST with cleaner logic and better handling
 * of boundary lines and empty lines between comments.
 *
 * @param root - The root AST node containing the parsed data file
 * @param settings - Configuration options for heading inference
 * @returns The extracted heading as a string, or null if no heading is found
 */
export function inferHeadingFromASTImproved(
  root: RootNode,
  settings: HeadingInferenceConfig = {},
): string | null {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if (isCommentNode(node)) {
      ...
      if (heading && nextNode && isCommentNode(nextNode)) {
        if (allowEmptyLinesBetweenComments) {
          heading = `${heading}${node.raw}\n`;
        } else {
          headingEndNodeIndex = i;
          break;
