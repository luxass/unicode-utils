...
const ucdFiles = await mapUCDFiles("v4.1");

describe("heading inference v4.1", async () => {
...
it("inferHeading(Unihan.txt)", () => {
    const content = ucdFiles.file("Unihan.txt");
    const expected = ucdFiles.expected("Unihan.txt.comments.txt");
