...
const ucdFiles = await mapUCDFiles("6.3.0");

describe("heading inferrence v6.3.0", async () => {
  ...
  it("inferHeading(IndicMatraCategory)", () => {
    const content = ucdFiles.file("IndicMatraCategory.txt");
    const expected = ucdFiles.file("IndicMatraCategory.comments.txt");
